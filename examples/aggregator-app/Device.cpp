...
DeviceEntityLocation::DeviceEntityLocation(const char * szDeviceName, std::string szLocation, std::string entityID,
                                           std::string entityLocation) : Device(szDeviceName, szLocation)
{
    strcpy(mEntityID, entityID.c_str());
    strcpy(mEntityLocationAttribute, entityLocation.c_str());
}

std::string DeviceEntityLocation::GetEntityID()
{
    return std::string(mEntityID);
}

void DeviceEntityLocation::SetEntityID(std::string entityID)
{
    if (strncmp(mEntityID, entityID.c_str(), sizeof(mEntityID)) != 0)
    {
        ChipLogProgress(DeviceLayer, "DeviceEntityLocation[%s]: EntityID changed from '%s' to '%s'", mName, mEntityID,
                        entityID.c_str());
        strcpy(mEntityID, entityID.c_str());
        if (mChanged_CB)
        {
            mChanged_CB(this, DeviceEntityLocation::kChanged_ID);
        }
    }
}

std::string DeviceEntityLocation::GetEntityLocation()
{
    return std::string(mEntityLocationAttribute);
}

void DeviceEntityLocation::SetEntityLocation(std::string entityLocation)
{
    if (strncmp(mEntityLocationAttribute, entityLocation.c_str(), sizeof(mEntityLocationAttribute)) != 0)
    {
        ChipLogProgress(DeviceLayer, "DeviceEntityLocation[%s]: EntityLocation changed from '%s' to '%s'", mName,
                        mEntityLocationAttribute, entityLocation.c_str());
        strcpy(mEntityLocationAttribute, entityLocation.c_str());
        if (mChanged_CB)
        {
            mChanged_CB(this, DeviceEntityLocation::kChanged_LocationAttribute);
        }
    }
}

void DeviceEntityLocation::SetChangeCallback(DeviceCallback_fn aChanged_CB)
{
    mChanged_CB = aChanged_CB;
}

void DeviceEntityLocation::HandleDeviceChange(Device * device, Device::Changed_t changeMask)
{
    if (mChanged_CB)
    {
        mChanged_CB(this, static_cast<DeviceEntityLocation::Changed_t>(changeMask));
    }
}
