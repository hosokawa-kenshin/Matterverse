...
class DeviceEntityLocation : public Device {
public:
    static const int kEntityIDSize = 64;
    static const int kEntityLocationSize = 64;

    enum Changed_t {
        kChanged_ID = kChanged_Last << 1,
        kChanged_LocationAttribute = kChanged_Last << 2,
    } Changed;

    DeviceEntityLocation(const char * szDeviceName, std::string szLocation, std::string entityID, std::string entityLocation);

    std::string GetEntityID();
    void SetEntityID(std::string entityID);
    std::string GetEntityLocation();
    void SetEntityLocation(std::string entityLocation);

    using DeviceCallback_fn = std::function<void(DeviceEntityLocation *, DeviceEntityLocation::Changed_t)>;
    void SetChangeCallback(DeviceCallback_fn aChanged_CB);
...
private:
    char mEntityID[kEntityIDSize + 1] = { 0 };
    char mEntityLocationAttribute[kEntityLocationSize + 1] = { 0 };
    DeviceCallback_fn mChanged_CB;
};
...
public:
    ComposedDevice(const char * szDeviceName, std::string szLocation)
