...
void AppTask::SwitchActionEventHandler(AppEvent * aEvent)
{
    ...
    {
        // Example entry data for RecordEntry command
        // Format: BeaconUUID:MajorID:MinorID:MediatorUID
        const char * exampleEntry = "12345678-1234-5678-1234-567812345678:00100:00200:blescanner-esp32";
        
        BindingCommandData * data = Platform::New<BindingCommandData>();
        data->commandId           = 0x00; // RecordEntry command ID
        data->clusterId           = 0xFFF1FC01; // Location Detector cluster ID
        
        // Allocate memory for entry data
        size_t entryLen = strlen(exampleEntry) + 1;
        char * entryCopy = static_cast<char*>(chip::Platform::MemoryAlloc(entryLen));
        if (entryCopy != nullptr)
        {
            memcpy(entryCopy, exampleEntry, entryLen);
